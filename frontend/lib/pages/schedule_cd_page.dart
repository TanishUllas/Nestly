...
class ScheduleCDPage extends StatefulWidget {
  ...
  const ScheduleCDPage({super.key, required this.userId});
...
class _ScheduleCDPageState extends State<ScheduleCDPage> {
  ...
  // ✅ **Pre-Approve Cab/Delivery**
  Future<void> preApproveCD() async {
    ...
    if (response.statusCode == 200) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("✅ Cab/Delivery Pre-Approved"), backgroundColor: Colors.green),
      );
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("❌ Failed to Pre-Approve"), backgroundColor: Colors.red),
      );
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('Schedule Cab/Delivery', style: TextStyle(color: Colors.white)),
        leading: IconButton(
      ...
      body: Container(
        ...
        child: Column(
          children: [
            ...
            // ✅ **Pre-Approval Button**
            ElevatedButton(
              onPressed: isProcessing ? null : preApproveCD,
              style: _buttonStyle(),
              child: isProcessing
